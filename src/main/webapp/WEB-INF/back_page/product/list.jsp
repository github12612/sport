...
function isShow(pageNo,name,brandId,isShow) {
	...
	$("#jvForm").attr("action","/product/isShow.do?pageNo="+pageNo+"&name="+name+"&brandId="+brandId+"&isShow="+isShow).submit();
}
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${page.list}" var="product">
			<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
				...
				<td align="center"><c:if test="${product.isNew==0}">否</c:if><c:if test="${product.isNew==1}">是</c:if></td>
...
<div style="margin-top:15px;"><input class="del-button" type="button" value="删除" onclick="optDelete();"/><input class="add" type="button" value="上架" onclick="isShow('${page.pageNo}','${name}','${brandId}','${isShow}');"/><input class="del-button" type="button" value="下架" onclick="optDelete();"/></div>
</form>
</div>
</body>
</html>