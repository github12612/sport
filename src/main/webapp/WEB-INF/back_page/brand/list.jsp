...
<table cellspacing="1" cellpadding="0" border="0" width="100%" class="pn-ltable">
	...
	<tbody class="pn-ltbody">
		<c:forEach items="${page.list}" var="page">
			...
			<td align="center"><img width="40" height="40" src="${page.allUrl}"/></td>
			<td align="center">${page.description}</td>
			<td align="center">${page.sort}</td>
			<td align="center"><c:if test="${page.isDisplay == 1}">是</c:if><c:if test="${page.isDisplay == 0}">不是</c:if></td>
			<td align="center">
			<a class="pn-opt" href="javascript:void(0)" onclick="window.location.href='/brand/toEditor.do?id=${page.id}'">修改</a> | <a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;} window.location.href='/brand/delete.do?id=${page.id}&name=${name}&isDisplay=${isDisplay}'" href="javascript:void(0)">删除</a>
			</td>
		</tr>
