...
//结算
function trueBuy(){
 	window.location.href = "/buyer/toBuy.shtml";
}

//清除购物车
function clearCart(){
	 window.location.href="/shopping/clean.shtml";
}
//-
...
function delProduct(skuId){
	if(confirm("确定要删除吗")){
		window.location.href='/shopping/delete.shtml?skuId='+skuId;
	}
}
</script>
</head>
			...
			<c:forEach items="${buyCart.items}" var="item">
				<tr>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a class="pic" title="${item.sku.product.name}" href="javascript:void(0)">
								<img alt=" ${item.sku.product.name}" src="${item.sku.product.img.allUrl}"></a>
								<dl>
					...
					<td class="blue">
						<a onclick="delProduct(${item.sku.id})" title="删除" href="javascript:void(0);">删除</a>
					</td>
				</tr>
			</c:forEach>	
			</tbody>
			...
			<div class="page">
				...
				<span class="r box_gray">
					<dl class="total">
						...
						<dd><em class="l">运费：</em>￥<var id="fee">${buyCart.frr}</var>元</dd>
